...
class MultiChanFeaExtra_Block(nn.Module):
    def __init__(self, input_channel=1, output_channel=32, cha_num=19):
        super(MultiChanFeaExtra_Block, self).__init__()
...
class SingleChanFeaExtra_Block(nn.Module):
    def __init__(self, eeg_length=1000):
        super(SingleChanFeaExtra_Block, self).__init__()
        self.feature_extractors = TransformerEXt(eeg_length)

        
    def forward(self, x):
        feature_list = []
        for i in range(x.size(2)):  # 遍历第二维
            feature = x[:, :, i, :]  # 获取当前特征
...
class FusionRecon_Block(nn.Module):
    def __init__(self):
        super(FusionRecon_Block, self).__init__()
...
class ASTI_Net(nn.Module):
    def __init__(self, eeg_length=1280, cha_num=19, input_channel=1, fea_channel=32, kernel_size=3, padding=1):
        ...
        self.mcfe = MultiChanFeaExtra_Block(input_channel, fea_channel, cha_num)
        self.scfe = SingleChanFeaExtra_Block(eeg_length)
        self.re = FusionRecon_Block()
