...
class Recon_Block_Half(nn.Module):
    def __init__(self):
        super(Recon_Block_Half, self).__init__()
        self.conv2 = nn.Conv2d(in_channels=32, out_channels=16, kernel_size=(1, 5), padding=(0, 2), bias=False)
        self.bn2 = nn.BatchNorm2d(16)
        self.conv3 = nn.Conv2d(in_channels=16, out_channels=8, kernel_size=(1, 5), padding=(0, 2), bias=False)
        self.bn3 = nn.BatchNorm2d(8)
...
class ASTI_Net_Without_SC(nn.Module):
    def __init__(self, eeg_length=1280, cha_num=19, input_channel=1, fea_channel=32, kernel_size=3, padding=1):
        super(ASTI_Net_Without_SC, self).__init__()
        self.kernel_size = kernel_size
        self.padding = padding

        self.mcfe = MCFE_Block(input_channel, fea_channel, cha_num)
        self.re = Recon_Block_Half()
...
class ASTI_Net_Without_MC(nn.Module):
    def __init__(self, eeg_length=1280, cha_num=19, input_channel=1, fea_channel=32, kernel_size=3, padding=1):
        super(ASTI_Net_Without_MC, self).__init__()
        self.kernel_size = kernel_size
        self.padding = padding
...
class ASTI_Net_Without_FR(nn.Module):
    def __init__(self, eeg_length=1280, cha_num=19, input_channel=1, fea_channel=32, kernel_size=3, padding=1):
        super(ASTI_Net_Without_FR, self).__init__()
        self.kernel_size = kernel_size
        self.padding = padding
        
        self.mcfe = MCFE_Block(input_channel, fea_channel, cha_num)
        self.scfe = SCFE_Block(eeg_length)
        self.block1 = nn.Conv2d(in_channels=fea_channel * 2, out_channels=input_channel, kernel_size=kernel_size, padding=padding, bias=False)

    def forward(self, x):
        x1 = self.mcfe(x)
        x2 = self.scfe(x)
        x3 = torch.cat((x1, x2), dim=1)
        eeg_re = self.block1(x3)
